...
def main():
    ...
    config.use_ce_loss = data_args.use_ce_loss
    logger.info(f'Use CE loss: {config.use_ce_loss}')
    ...
    if not (data_args.use_ce_loss or enable_numeric_aux or enable_angle_aux or enable_coord_dist_aux or enable_action_hinge_aux):
        raise ValueError(
            'At least one training loss must be enabled. '
            'Set use_ce_loss=true or enable an auxiliary loss with a positive lambda.'
        )
