...
@registry.register_dataset(name="ObjectNav-HM3D")
class ObjectNavDatasetHM3D(PointNavDatasetV1):
    ...
    def from_json(
        ...
        for i, episode in enumerate(deserialized["episodes"]):
            ...
            # Filter out episodes based on certain criteria

            # 1. Filter by object category (cheap string lookup)
            filtered_goals = {"foodstuff", "stationery", "fruit", "plaything", "hand_tool", "game_equipment",
                              "kitchenware"}
            if episode.object_category in filtered_goals:
                continue  # skip episode if the goal is in the excluded list

            # 2. Filter out episodes where no goal is on the same vertical level (within 0.5m)
            start_y = episode.start_position[1]
            has_nearby_goal = False

            for goal in episode.goals:
                if goal.view_points:
                    goal_y = goal.view_points[0].agent_state.position[1]
                    if abs(start_y - goal_y) <= 0.5:
                        has_nearby_goal = True
                        break  # Found a valid nearby goal, no need to check further

            if not has_nearby_goal:
