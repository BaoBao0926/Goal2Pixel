...
from task_patch.ovon_register.attributes import AgentStateSpec, ReplayActionSpec, OVONEpisode, OVONObjectViewLocation

@registry.register_dataset(name="ObjectNav-OVON")
class ObjectNavDatasetOVON(PointNavDatasetV1):
    ...
    def from_json(self, json_str: str, scenes_dir: Optional[str] = None) -> None:
        ...
        for i, episode in enumerate(deserialized["episodes"]):
            ...
            episode.scene_id = episode.scene_id.replace('//', '/')
            if episode.scene_id.count('/') > 2:
                segmented = episode.scene_id.split('/')
                episode.scene_id = '/'.join(segmented[-3:])
