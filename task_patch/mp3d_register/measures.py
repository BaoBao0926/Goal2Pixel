"""
Adapt from https://github.com/jacobkrantz/VLN-CE/blob/master/habitat_extensions/measures.py
...
from task_patch.mp3d_register import maps
...
class OracleSuccess(Measure):
    ...
    def __init__(self, *args: Any, config, **kwargs: Any):
        self._config = config
        super().__init__()
...
from task_patch.mp3d_register.dataset import RxRVLNCEDatasetV1
...
class NDTW(Measure):
    ...
    def __init__(
        ...
        if "{role}" in config.gt_path:
            self.gt_json = {}
            roles_to_try = self._resolve_ndtw_roles(config)
            loaded_roles = []
            missing_role_paths = []

            for role in roles_to_try:
                gt_path = config.gt_path.format(split=config.split, role=role)
                if not os.path.exists(gt_path):
                    missing_role_paths.append(gt_path)
                    continue
            ...
            if len(loaded_roles) == 0:
                raise FileNotFoundError(
                    "Could not load any NDTW GT file for roles "
                    f"{roles_to_try}. Missing paths: {missing_role_paths}"
                )

            if len(missing_role_paths) > 0:
                logger.warning(
                    "NDTW skipped missing GT files for roles not used by this run: %s",
                    missing_role_paths,
                )
        else:
            with gzip.open(
        ...
        # split_candidates = self._build_split_candidates(config.split)
        # loaded_split = None
        ...
        #                 gt_path = config.gt_path.format(split=split_name, role=role)
        #                 with gzip.open(gt_path, "rt") as f:
        #                     gt_json.update(json.load(f))
        #             self.gt_json = gt_json
        #         else:
        #             gt_path = config.gt_path.format(split=split_name)
        #             with gzip.open(gt_path, "rt") as f:
        #                 self.gt_json = json.load(f)
        ...
        #     raise FileNotFoundError(
        #         f"Could not load NDTW gt_path for any split candidate: {split_candidates}"
        #     )
        # if loaded_split != config.split:
        #     logger.warning(
        #         "NDTW split fallback: requested '%s', loaded '%s' from gt_path",
        #         config.split,
        #         loaded_split,
        #     )
    ...
    @staticmethod
    def _resolve_ndtw_roles(config: DictConfig) -> List[str]:
        # Highest priority: roles explicitly provided in ndtw config.
        if "roles" in config and config.roles:
            return list(config.roles)
        if "role" in config and config.role:
            return [str(config.role)]

        # Safe default for guide-mode evaluation.
        return ["guide"]
    ...
    @staticmethod
    def _build_split_candidates(split_name: str) -> list:
        # Support custom subset names like val_unseen_eval200-en-US.
        candidates = [split_name]

        if split_name.startswith("val_unseen"):
            candidates.append("val_unseen")
        elif split_name.startswith("val_seen"):
            candidates.append("val_seen")
        elif split_name.startswith("train"):
            candidates.append("train")

        # Also try removing language suffixes like -en-US.
        base_no_lang = split_name.split("-")[0]
        if base_no_lang not in candidates:
            candidates.append(base_no_lang)
