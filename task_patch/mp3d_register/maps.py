...
"""
Adapted from https://github.com/jacobkrantz/VLN-CE/blob/729d141b2ee10628061ada74dd3a5b9f70faeba5/habitat_extensions/maps.py
"""
...
def draw_reference_path(
    ...
    for i, pt_to in enumerate(shortest_path_points):
        ...
        if i != 0:
            # filter out duplicate points which cause drawline to crash
            if pt_from == pt_to:
                # Known bug: duplicate path points collapse to zero-length segments, skip to avoid drawline crash.
                continue
            # end of known bug
            drawline(
                img,
...
def draw_mp3d_nodes(
    ...
    for node in graph:
        ...
        if abs(pos[1] - starting_height) < 1.0:
            ...
            if 0 <= r_x < img.shape[0] and 0 <= r_y < img.shape[1]:
                if img[r_x, r_y]:
