...
"""
Adapted from Uni-Goal pose_utils.py
"""
...
def threshold_pose_map(global_map, gx1, gx2, gy1, gy2):
    cx, cy = np.where(global_map > 0)
    local_map = np.zeros((gx2-gx1, gy2-gy1))
    ccx = np.clip(cx, gx1, gx2-1)
    ccy = np.clip(cy, gy1, gy2-1)
    local_map[ccx-gx1, ccy-gy1] = 1
    return local_map

def get_l2_dis_point_map(start, goal_map):
    cx, cy = np.where(goal_map > 0)
    dis = ((cx-start[0])**2 + (cy-start[1])**2)**0.5
    dis = dis.min()
    return dis
